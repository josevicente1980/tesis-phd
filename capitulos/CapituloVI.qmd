...
col_inter <- posibles_inter[
  posibles_inter %in% names(cgnac_base)
][1]
...
if (!is.na(col_inter)) {
  cgnac_base <- cgnac_base |>
    dplyr::filter(
      as.numeric(
        haven::zap_labels(.data[[col_inter]])
      ) != 1
    )
}
...
tabla_usc(
  ...
  nota_tabla(
    nota = paste(
      "La tabla no dispone de factor de expansión,",
      "ya que corresponde a información obtenida",
      "mediante estudio de casos según la metodología",
      "de la ESPAC."
    ),
    fuente = fuente_espac
  )
...
colores_usos <- c(
  "Cultivos permanentes" = unname(paleta_figuras_tesis["principal"]),
  "Cultivos transitorios" = unname(paleta_figuras_tesis["secundario"]),
  "Pastos cultivados" = unname(paleta_figuras_tesis["acento"]),
  "Pastos naturales" = unname(paleta_figuras_tesis["apoyo"]),
  "Barbecho y descanso" = unname(paleta_figuras_tesis["neutro"])
)
...
La @tbl-tenencia-sector presenta la distribución promedio del área de las unidades de producción agropecuaria (UPA) según los principales regímenes de tenencia de la tierra y el sector productivo dominante. Para su construcción se identificó primero la actividad productiva predominante en cada UPA a partir del rubro con mayor valor económico de producción entre las actividades reportadas en la encuesta. Posteriormente, estas actividades se agruparon en cuatro sectores productivos (agricultura, pecuario, forestal y otros) y se calculó la proporción promedio del área del predio bajo cada régimen de tenencia (propiedad, arrendamiento y tenencias especiales) para cada grupo de UPA.
